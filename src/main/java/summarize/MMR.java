...
public class MMR {
	...
	public String[] summaryMMR(Map<String, Double> sentenceMap,
			Map<String, Double> termMap, Map<String, int[]> sentencePosMap) {
		...
		for (int i = 0; i < sentenceMap.size() * 0.05 - 1; i++) {
			...
			for (int j = 0; j < selectingSen.size(); j++) {
				mar[j] = lambda * sentenceMap.get(selectingSen.get(j))
						- (1 - lambda)
						* (marginal(selectingSen.get(j), selectedSen, termMap));
			}
	...
	public double marginal(String sentence, List<String> selectedSen,
			Map<String, Double> termMap) {
		return calSimilarity(sentence, selectedSen, termMap);
	}

	// 计算两个句子的相似度
	public double calSimilarity(String sentence1, List<String> selectedSen,
			Map<String, Double> termMap) {
	...
	// 对句子按时间顺序重新排序
	public String[] reordering(List<String> sentences,
			Map<String, int[]> sentencePosMap) {
