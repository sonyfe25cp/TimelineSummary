...
public class LSA {
	...
	//构建词-句子矩阵
	public Matrix constructMatrix(Map<String,Double> termMap,List<String> sentences){
		int m=termMap.size();
	...
	public Matrix calSVD(Matrix matrix){
		...
		Matrix v=svd.getV().getMatrix(0, matrix.getColumnDimension()-1, 0, d);//右矩阵
	...
	public String[] summaryLSA(Matrix V,List<String> sentences){
		...
		for(int i=0;i<sentences.size()*0.05;i++){
			for(int j=0;j<sentences.size();j++){
				double[] column=V.getMatrix(0, n-1, j, j).getColumnPackedCopy();
