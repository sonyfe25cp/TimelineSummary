...
public class TRM1 {
	...
	public Map<String,Double> constructTRM(Matrix matrix,List<String> sentences){
		...
		for(int i=0;i<senLength;i++){
			for(int j=i+1;j<senLength;j++){
				simArray[count++]=calSimilarity(matrix.getMatrix(0, wordLength-1, i, i).getColumnPackedCopy(),
						matrix.getMatrix(0, wordLength-1, j, j).getColumnPackedCopy());
			}
		}
		for(int i=0;i<senLength;i++){
			double sumSim=0.0;
